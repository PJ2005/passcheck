...
// SeedMockPhonePeData seeds matching dummy PhonePe vendor txns
func SeedMockPhonePeData(ctx context.Context, db *pgxpool.Pool, merchantID string, vendorIntegrationID string) error {
	log.Printf("[DEMO] Starting Mock PhonePe Data Seeder for Merchant %s", merchantID)
	...
	// Step 2: Create Matching records
	for rows.Next() {
		var utr string
		var amount float64
		var txnDate time.Time
		...
		vendorTxns = append(vendorTxns, vendors.StandardVendorTxn{
			VendorTxnID:    fmt.Sprintf("pp_demo_%s", utr[len(utr)-6:]),
			Amount:         amount,
			UTRNumber:      utr,
			SettlementDate: txnDate,
			VendorName:     "PhonePe",
	...
	for i := 1; i <= 3; i++ {
		...
		vendorTxns = append(vendorTxns, vendors.StandardVendorTxn{
			VendorTxnID:    fmt.Sprintf("pp_pending_%d", rand.Intn(99999)),
			Amount:         amount,
			UTRNumber:      fmt.Sprintf("PENDING_PP_UTR_%d", rand.Intn(999999)),
			SettlementDate: time.Now(),
			VendorName:     "PhonePe",
		})
	}
	...
	log.Printf("[DEMO] Successfully seeded %d mock PhonePe transactions!", insertedCount)
	return nil
}
